...
/*@ requires \valid(context);
    requires \valid(key + (0..keySize-1));
    requires keySize == AES_KEY_SIZE_128;
    requires CIPHERS_MAX_KEY_SIZE < INT_MAX;
    ensures \result == CIPHER_ERR_INVALID_KEY_SIZE || \result == CIPHER_ERR_BAD_CONTEXT_SIZE || \result == CIPHER_INIT_SUCCESS;
    ensures \result == CIPHER_ERR_INVALID_KEY_SIZE ==> (keySize != AES_KEY_SIZE_128 || keySize != AES_KEY_SIZE_192 || keySize != AES_KEY_SIZE_256);
    ensures \result == CIPHER_ERR_INVALID_KEY_SIZE ==> (keySize == AES_KEY_SIZE_128 && !IS_USED(MODULE_CRYPTO_AES_128)) || (keySize == AES_KEY_SIZE_192 && !IS_USED(MODULE_CRYPTO_AES_192)) || (keySize == AES_KEY_SIZE_256 && !IS_USED(MODULE_CRYPTO_AES_256));
    ensures \result == CIPHER_ERR_BAD_CONTEXT_SIZE ==> CIPHER_MAX_CONTEXT_SIZE < keySize;
    ensures \result == CIPHER_INIT_SUCCESS ==> context->key_size == keySize;
    ensures \result == CIPHER_INIT_SUCCESS ==> (\forall integer i; 0 <= i < CIPHER_MAX_CONTEXT_SIZE ==> (i < keySize ==> context->context[i] == key[i]) || (i >= keySize ==> context->context[i] == key[i % keySize])); 
    ensures keySize < CIPHERS_MAX_KEY_SIZE ==> (\forall uint8_t i; i < CIPHERS_MAX_KEY_SIZE ==> context->context[i] == key[i % keySize]);
    assigns *context;
*/
...
int aes_init(cipher_context_t *context, const uint8_t *key, uint8_t keySize)
{
    ...
    if (keySize < CIPHERS_MAX_KEY_SIZE) {
        ...
        /*@
        loop invariant 0<= i <=CIPHERS_MAX_KEY_SIZE;
        loop assigns i;
        loop variant CIPHERS_MAX_KEY_SIZE-i;
        */
...
/*@ requires \valid(key);
    requires \valid(key->rd_key+(0..14));
    requires \valid(userKey + (0..((bits/8)-1)));
    requires bits == 128 || bits == 192 || bits == 256;
    assigns *userKey,*key;
    behavior invalid_arguments:
        assumes !(\valid(userKey + (0..(bits/8)-1))) || !\valid(key) || (bits != 128 && bits != 192 && bits != 256);
        ensures \result == -1 || \result == -2;
    behavior invalid_arguments1:
        assumes !(\valid(userKey + (0..(bits/8)-1))) || !\valid(key);
        ensures \result == -1;
    behavior invalid_arguments2:
        assumes (bits != 128 && bits != 192 && bits != 256);
        ensures \result == -2;
                               ...
                               AES_KEY *key)
{
    ...
    if (bits == 128) {
        key->rounds = 10;
        //@ assert key->rounds == 10;
    }
    else if (bits == 192) {
        key->rounds = 12;
        //@ assert key->rounds == 12;
    }
    else {
        key->rounds = 14;
        //@ assert key->rounds == 14;
    }
                               ...
                               AES_KEY *key)
{
    ...
    /*@
       loop invariant 0 <= i < 4*(key->rounds);
...
/*@ 
    requires \valid(context);
    requires \valid(plainBlock);
    requires \valid(cipherBlock + (0..15));
    requires \separated(context, plainBlock + (0..15), cipherBlock + (0..15));
    requires AES_KEY_SIZE(context) == 16 || AES_KEY_SIZE(context) == 24 || AES_KEY_SIZE(context) == 32;
    assigns *cipherBlock,*plainBlock,*context;
    ensures \result == -2 || \result == -1 || \result == 1;
    ensures  \result == 0 ==> cipherBlock[0..15] == \old(plainBlock[0..15]);
*/
...
/*@ 
    requires \valid(context);
    requires \valid(cipherBlock);
    requires \valid(plainBlock);
    requires \separated(context, cipherBlock, plainBlock);
    requires AES_KEY_SIZE(context) == 16 || AES_KEY_SIZE(context) == 24 || AES_KEY_SIZE(context) == 32;
    requires cipherBlock[0..15] != plainBlock[0..15];
    requires \separated(context, plainBlock + (0..15), cipherBlock + (0..15));
    assigns *context, *cipherBlock ,*plainBlock;
    ensures \result == -1 || \result == -2 || \result == 1;
    ensures  \result == 0 ==> cipherBlock[0..15] == \old(plainBlock[0..15]);

    behavior null_context:
        assumes context->context == NULL;
        ensures \result == -1;

    behavior invalid_key_size:
        assumes AES_KEY_SIZE(context) != 16 && AES_KEY_SIZE(context) != 24 && AES_KEY_SIZE(context) != 32;
        ensures \result == -1;
                ...
                uint8_t *plainBlock)
{
    ...
    //@ assert \separated(rk, &s0, &s1, &s2, &s3, &t0, &t1, &t2, &t3);
