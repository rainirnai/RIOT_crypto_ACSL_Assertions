...
/*@ requires \valid(source+(0..15));
    requires \valid(dest+(0..15));
    requires \separated(source+(0..15), dest+(0..15));
    assigns dest[0..15],source[0..15];
*/
...
/*@ 
    requires n >= 0;
    ensures 0 <= \result <= SIZE_MAX;
    ensures (n==0) ==> \result == SIZE_MAX;
*/
...
/*@
  requires \valid(l_zero+(0..15));
  requires \valid(output+(0..15));
  assigns output[0..15],l_zero[0..15];
*/

static void calculate_l_i(const uint8_t l_zero[16], size_t i, uint8_t output[16])
{
    //@ assert \valid(output+(0..15));
    //@ assert \valid(l_zero+(0..15));
    memcpy(output, l_zero, 16);
    //@ assert \forall integer i; 0 <= i <= 15 ==> output[i] == (l_zero[i]);
    while ((i--) > 0) {
        double_block(output, output);
    }
}

/*@ requires \valid(block1 +(0..15));
    requires \valid(block2+(0..15));
    requires \valid(output+(0..15));
    assigns output[0..15],block1[0..15],block2[0..15];
*/
                      ...
                      uint8_t output[16])
{
    ...
    //@ assert \forall integer i; 0 <= i < 16 ==> output[i] == (block1[i] ^ block2[i]);
}

/*@
  requires \valid(state);
  requires \valid(state->l_zero+(0..15));
  requires \valid(state->offset+(0..15));
  requires \valid(state->cipher);
  requires \valid(input+(0..15));
  requires \valid(output+(0..15));
  requires mode == OCB_MODE_ENCRYPT || mode == OCB_MODE_DECRYPT;
  requires \separated(state->l_zero+(0..15), state->offset+(0..15), 
                      input+(0..15), output+(0..15), 
                      state->checksum+(0..15));
  assigns *state, *input, *output;
*/
...
/*@
    requires \valid(state);
    requires \valid(state->cipher);
    requires state->l_star != NULL;
    requires state->l_zero != NULL;
    requires \valid(data + (0 .. data_len - 1));
    requires \valid(output + (0 .. 15));
    requires data_len >= 0;
    assigns *output, *state, *data;
*/
                 ...
                 uint8_t output[16])
{
    ...
    for (size_t i = 0; i < m; ++i) {
        ...
        //@assert \forall integer i; 0 <= i < 16 ==> cipher_input[i] == (data[i] ^ offset[i]);
        
        state->cipher->interface->encrypt(&(state->cipher->context),
                                          cipher_input, enciphered_block);
        xor_block(output, enciphered_block, output);
        //@assert \forall integer i; 0 <= i < 16 ==> output[i] == (output[i] ^ enciphered_block[i]);
    ...
    if (remaining_data_len > 0) {
        ...
        //@ assert \valid(cipher_input+(0..remaining_data_len-1));
        //@ assert \valid(data+(0..remaining_data_len-1));
        ...
        state->cipher->interface->encrypt(&(state->cipher->context),
                                          cipher_input, enciphered_block);
        xor_block(output, enciphered_block, output);
        //@ assert \forall integer i; 0 <= i < 16 ==> offset[i] == (offset[i] ^ l_i[i]);
    }
}
...
/*@ 
    ...
    requires \valid(state->cipher);
    requires state->l_star != NULL;
    requires state->l_dollar != NULL;
    requires state->l_zero != NULL;
    requires nonce_len <= 15;
    requires tag_len >= 1 && tag_len <= 16;
    requires nonce != \null ==> \valid_read(nonce+(0..nonce_len-1));
    requires state != \null ==> \valid(state);
    assigns *state, *cipher,*nonce;
@*/
                     ...
                     ocb_state_t *state)
{
    ...
    //@ assert \valid((nonce_padded +16 - nonce_len) +(0..nonce_len-1));
    ...
    uint8_t stretch[24];
    //@ assert \valid(stretch+(0..15));
    //@ assert \valid(ktop+(0..15));
...
/*@ 
    ...
    requires \valid(tag+(0..15));
    requires \valid(nonce);
    requires \valid(input+(0..15));
    requires \valid(output+(0..15));
    requires \valid(nonce+(0..nonce_len-1));
    requires mode == OCB_MODE_ENCRYPT || mode == OCB_MODE_DECRYPT;
    ensures \result == -3 || \result == -4 || \result == -2 || \result >= 0 ;
@*/
                       ...
                       uint8_t *output, uint8_t mode)
{
    ...
    if (remaining_input_len > 0) {
        ...
        //@ assert \valid(final_block+(0..remaining_input_len-1));
        //@ assert \valid(pad+(0..remaining_input_len-1));
        memcpy(final_block, pad, remaining_input_len);
        for (uint8_t i = 0; i < remaining_input_len; ++i) {
            final_block[i] = input[i] ^ pad[i];
        }
        //@ assert \valid((output+output_pos)+(0..remaining_input_len-1));
        //@ assert \valid(final_block+(0..remaining_input_len-1));
        ...
        if (mode == OCB_MODE_ENCRYPT) {
            //@ assert \valid(padded_block+(0..remaining_input_len-1));
            //@ assert \valid(input+(0..remaining_input_len-1));
...
/*@ requires \valid(cipher);
    requires \valid(auth_data);
    requires auth_data_len >= 0 && auth_data_len < INT32_MAX;
    requires tag_len > 0 && tag_len <= 16;
    requires \valid(nonce);
    requires nonce_len >= 0 && nonce_len < 16;
    requires \valid(input);
    requires input_len >= 0 && input_len <= INT32_MAX - tag_len;
    requires \valid(output);
    requires tag_len % 2 == 0;
    requires tag_len * 8 <= cipher->interface->block_size;
    assigns *cipher, *auth_data, *nonce, *input, *output;
*/
                           ...
                           uint8_t *output)
{
    ...
    //@ assert \valid((output+cipher_text_length)+(0..tag_len-1));
    //@ assert \valid(tag+(0..tag_len-1));
...
/*@
  requires \valid(cipher);
  requires \valid(auth_data);
  requires auth_data_len >= 0 && auth_data_len < UINT32_MAX;
  requires tag_len >= 0 && tag_len <= 16;
  requires \valid(nonce);
  requires nonce_len > 0 && nonce_len < UINT32_MAX;
  requires \valid(input);
  requires input_len >= tag_len && input_len < INT32_MAX + tag_len;
  requires \valid(output);
  requires \separated(auth_data + (0..auth_data_len-1), nonce + (0..nonce_len-1),
                      input + (0..input_len-1), output + (0..input_len-tag_len-1));
  assigns *cipher,*auth_data,*nonce,*input,*output;
  ensures \result >= -6 && \result <= INT32_MAX;
*/
                           ...
                           uint8_t *output)
{
    ...
    /* Check the tag */
    //@ assert \valid(tag+(0..tag_len-1));
    //@ assert \valid((input+input_len-tag_len)+(0..tag_len-1));
    ...
    //@ assert \forall integer i; 0 <= i < input_len - tag_len ==> output[i] == (input[i]);
}
